$(document).ready( function() {		
	...
	$.ajax({
		'url' : base_url + 'modificar_cotizacion/obtener_datos_iniciales',
		'type' : 'post',
	...
	function cargar_cotizacion_temporal() {
		$.ajax({
			'url' : base_url + 'modificar_cotizacion/cargar_cotizacion_temporal',
			'type' : 'post',
			'success' : function(obj){
				...
				if(obj['tipo_cotizacion'] == "existente") {
					...
					$('#fecha_cotizacion_guardada').html(cotizacion_temporal.fecha_edicion);
					$('#alerta_cotizacion').removeClass('d-none');
					$('#folio_cotizacion').val(cotizacion_temporal.id_cotizacion_temporal);
					...
					$('#mostrar_roi [value="' + cotizacion_temporal.mostrar_roi + '"').prop("selected", true).change();
					$('#num_servicio').val(cotizacion_temporal.numero_servicio);			
					
					if(cotizacion_temporal.forma_calculo != null) {
						$('#forma_calculo [value="' + cotizacion_temporal.forma_calculo + '"').prop("selected", true);
						$('#tarifa [value="' + cotizacion_temporal.tarifa + '"').prop("selected", true);
						$('#periodo [value="' + cotizacion_temporal.periodo + '"').prop("selected", true);
						$('#consumo_promedio_kwh').val(cotizacion_temporal.consumo_promedio_kwh);
						$('#consumo_promedio_pesos').val(cotizacion_temporal.consumo_promedio_precio);
						
						if(cotizacion_temporal.periodo == 'bimestral') {
							$("label[for='consumo_promedio_kwh']").text("Consumo promedio bimestral (kWh)");
							$("label[for='consumo_promedio_pesos']").text("Consumo promedio bimestral ($)");
							...
							$("label[for='consumo_promedio_pesos']").text("Consumo promedio mensual ($)");
						...
						switch(cotizacion_temporal.forma_calculo) {
							...
							case 'recibo' :
								...
								$('#periodo_6').val(cotizacion_temporal.recibo_6).trigger('input');
						...
						$('#tipo_interconexion [value="' + cotizacion_temporal.tipo_interconexion + '"').prop("selected", true).change();
					}

					if(cotizacion_temporal.paneles_elegidos == 1) {
						$('#potencia_total').val(cotizacion_temporal.potencia_total);
						$('#produccion_promedio').val(cotizacion_temporal.produccion_periodo);
					...
					$('#admin_gasto_bimestral').val(dinero(cotizacion_temporal.gasto_bimestral_cfe, false));
					$('#admin_gasto_anual').val(dinero(cotizacion_temporal.gasto_anual_cfe, false));
					$('#admin_costo_por_metro').val(cotizacion_temporal.costo_metro);
					$('#admin_costo_material_electrico').val(cotizacion_temporal.material_electrico);
					$('#admin_costo_instalacion').val(cotizacion_temporal.instalacion);
					
					if(totales != false) {
						console.log("CARGA TEMPORAL PRODUCTOS:");
						console.log(obj['productos_temporales']);
						elementos_cotizacion = obj['productos_temporales'];
						console.log("TOTALES TEMPORAL:");
						console.log(obj['totales']);
						totales = obj['totales'];
	...
	$('#nombre_usuario').change(function() {
		...
		if(nombre) {
			...
			$.ajax({
				'url' : base_url + 'modificar_cotizacion/guardar_nombre',
				'type' : 'post',
				'data' : {
	...
	$('#ubicacion').change(function() {
		...
		if(ubicacion) {
			$(this).removeClass('is-invalid');
			$.ajax({
				'url' : base_url + 'modificar_cotizacion/guardar_ubicacion',
				'type' : 'post',
				'data' : {
					'id_cotizacion_temporal' : id_cotizacion_temporal,
					'ubicacion' : ubicacion
	...
	$('#telefono').change(function() {
		...
		if(telefono) {
			...
			$(this).removeClass('is-invalid');
			$.ajax({
				'url' : base_url + 'modificar_cotizacion/guardar_telefono',
				'type' : 'post',
				'data' : {
					'id_cotizacion_temporal' : id_cotizacion_temporal,
					'telefono' : telefono
	...
	$('#correo').change(function() {
		...
		if(correo) {
			$(this).removeClass('is-invalid');
			$.ajax({
				'url' : base_url + 'modificar_cotizacion/guardar_correo',
				'type' : 'post',
				'data' : {
					'id_cotizacion_temporal' : id_cotizacion_temporal,
					'correo' : correo
	...
	$('#num_servicio').change(function() {
		...
		if(num_servicio) {
			$(this).removeClass('is-invalid');
			$.ajax({
				'url' : base_url + 'modificar_cotizacion/guardar_num_servicio',
				'type' : 'post',
				'data' : {
					'id_cotizacion_temporal' : id_cotizacion_temporal,
	...
	$('#fecha').change(function() {
		...
		if(fecha) {
			$(this).removeClass('is-invalid');
			$.ajax({
				'url' : base_url + 'modificar_cotizacion/guardar_fecha',
				'type' : 'post',
				'data' : {
					'id_cotizacion_temporal' : id_cotizacion_temporal,
					'fecha' : fecha
	...
	$('#mostrar_roi').change(function() {
		...
		$.ajax({
			'url' : base_url + 'modificar_cotizacion/guardar_mostrar_roi',
			'type' : 'post',
			'data' : {
				'id_cotizacion_temporal' : id_cotizacion_temporal,
	...
	$('#tipo_interconexion').change(function() {
		...
		$(this).removeClass('is-invalid');
		$.ajax({
			'url' : base_url + 'modificar_cotizacion/guardar_tipo_interconexion',
			'type' : 'post',
			'data' : {
				'id_cotizacion_temporal' : id_cotizacion_temporal,
	...
	function limpiar_inputs(forma_calculo) {
		if(!$('#forma_calculo option:selected').prop("disabled") && !$('#tarifa option:selected').prop("disabled")) {
			$('#periodo_1, #periodo_2, #periodo_3, #periodo_4, #periodo_5, #periodo_6, #consumo_promedio_kwh, #consumo_promedio_pesos').val('').prop("readonly", true);
	...
	$('#calcular_consumos').click(function() {
		...
		if($('#forma_calculo').val() == 'consumo' || $('#forma_calculo').val() == 'recibo') {
			$.ajax({
				'url' : base_url + 'modificar_cotizacion/calcular_precio_promedio',
				'type' : 'post',
				'data' : {
					'id_cotizacion_temporal' : id_cotizacion_temporal,
			...
			$.ajax({
				'url' : base_url + 'modificar_cotizacion/calcular_consumo_promedio',
				'type' : 'post',
				'data' : {
					'id_cotizacion_temporal' : id_cotizacion_temporal,
	...
	function habilitar_inputs() {
		$('#tipo_interconexion, #tipo_panel, #tipo_estructura, #sistema_monitoreo, #instalacion_electrica, #optimizador, #microinversor, #inversor_central').prop("disabled", false); 
	}
	
	$('#tipo_panel').change(function() {
		$(this).removeClass('is-invalid');
		$.ajax({
			'url' : base_url + 'modificar_cotizacion/guardar_panel',
			'data' : {
				'id_panel' : $(this).val(),
				'id_cotizacion_temporal' : id_cotizacion_temporal
			},
	...
	$('#num_paneles').change(function() {
		...
		wto = setTimeout(function() {
			$.ajax({
				'url' : base_url + 'modificar_cotizacion/guardar_numero_paneles',
				'data' : {
	...
	$('#agregar_estructura').click(function() {
		$.ajax({
			'url' : base_url + 'modificar_cotizacion/guardar_producto',
			'data' : {
	...
	$('#inversor_central, #microinversor, #optimizador, #sistema_monitoreo').change(function() {
		let id = $(this).attr('id');
		let tipo_producto;
		let cantidad;
	...
	$('#instalacion_electrica, #admin_costo_instalacion').change(function() {
		...
		$.ajax({
			'url' : base_url + 'modificar_cotizacion/guardar_instalacion_electrica',
			'data' : {
				'id_cotizacion_temporal' : id_cotizacion_temporal,
				'metros' : metros,
	...
	$('#admin_costo_material_electrico').change(function() {
		...
		$.ajax({
			'url' : base_url + 'modificar_cotizacion/guardar_material_electrico',
			'data' : {
				'id_cotizacion_temporal' : id_cotizacion_temporal,
	...
	$('#cambiar_indice').click(function() {
		if(!cambiando_indice) {
			...
			if(indice_correcto) {
				...
				$.ajax({
					'url' : base_url + 'modificar_cotizacion/guardar_indice_utilidad',
					'data' : {
						'id_cotizacion_temporal' : id_cotizacion_temporal,
	...
	$('#cambiar_tasa_usd').click(function() {
		if(!cambiando_tasa) {
			...
			if(tasa_correcta) {
				...
				$.ajax({
					'url' : base_url + 'modificar_cotizacion/guardar_tasa_cambio',
					'data' : {
						'id_cotizacion_temporal' : id_cotizacion_temporal,
	...
	$(document).on('click', '.eliminar-producto', function() {
		...
		$.ajax({
			'url' : base_url + 'modificar_cotizacion/eliminar_producto',
			'data' : {
	...
	$(document).on('change', '.cantidad-producto', function() {
		...
		if(tipo_producto == "panel_solar" && principal == "1") {
			...
			wto = setTimeout(function() {
				$.ajax({
					'url' : base_url + 'modificar_cotizacion/cambiar_cantidad_producto',
					'data' : {
						'id_producto' : id_producto,
						'id_cotizacion_temporal' : id_cotizacion_temporal,
						'tipo_producto' : tipo_producto,
	...
	function mostrar_ahorros() {
		$('#periodo_tabla td:eq(0)').text(dinero(datos_ahorro.consumo_promedio_pesos));
		$('#1_anio td:eq(0)').text(dinero(datos_ahorro.pagos_anuales_sin_paneles[0]));
		$('#5_anios td:eq(0)').text(dinero(datos_ahorro.pagos_anuales_sin_paneles[4]));
		$('#10_anios td:eq(0)').text(dinero(datos_ahorro.pagos_anuales_sin_paneles[9]));
		$('#25_anios td:eq(0)').text(dinero(datos_ahorro.pagos_anuales_sin_paneles[24]));
		
		$('#periodo_tabla td:eq(1)').text(dinero(datos_ahorro.restante_pesos));
		$('#1_anio td:eq(1)').text(dinero(datos_ahorro.pagos_anuales_con_paneles[0]));
		$('#5_anios td:eq(1)').text(dinero(datos_ahorro.pagos_anuales_con_paneles[4]));
		$('#10_anios td:eq(1)').text(dinero(datos_ahorro.pagos_anuales_con_paneles[9]));
		$('#25_anios td:eq(1)').text(dinero(datos_ahorro.pagos_anuales_con_paneles[24]));
		...
		$('#retorno_inversion td:eq(0)').text(datos_ahorro.retorno_inversion + " años");
	}
	...
	function actualizar_lista_elementos() {
		...
		$.each(elementos_cotizacion, function(index, producto) {
			...
			if(producto.principal == 1) {
				switch(producto.tipo_producto) {
					case 'panel_solar' :
						$('#tipo_panel [value="' + producto.id_producto + '"').prop("selected", true);
						$('#num_paneles').prop("disabled", false);
						break;
					case 'inversor' :
						$('#inversor_central [value="' + producto.id_producto + '"').prop("selected", true);
						break;
					case 'microinversor' :
						$('#microinversor [value="' + producto.id_producto + '"').prop("selected", true);
						$('#cantidad_microinversor').prop('disabled', false).val(producto.cantidad);
						break;
					case 'optimizador' :
						$('#optimizador [value="' + producto.id_producto + '"').prop("selected", true);
						$('#cantidad_optimizador').prop('disabled', false).val(producto.cantidad);
						break;
					case 'sistema_monitoreo' :
						$('#sistema_monitoreo [value="' + producto.id_producto + '"').prop("selected", true);
						break;	
					case 'producto_fijo' :
						if(producto.datos_producto.codigo == "INGENIERIA_INSTALACION") {
							$('#admin_costo_tuberia').val(dinero(totales.tuberia_cableado, false));
							$('#instalacion_electrica').val(totales.metros_instalacion);
							$('#tabla_productos tbody tr[data-prod-id=' + producto.id_producto + '][data-tipo="producto_fijo"] input.cantidad-producto').prop("disabled", true);
			...
			let fila_costos = tabla_costos.row.add([
				producto.datos_producto.codigo,
				producto.datos_producto.producto,
				producto.datos_producto.marca,
				producto.cantidad,
			...
			tabla_totales +=
				'<tr>' +
					'<td>' + producto.datos_producto.codigo + '</td>' +
					'<td>' + producto.datos_producto.producto + '</td>' +
					'<td>' + producto.cantidad + '</td>' +
					'<td>' + dinero(producto.precio_final) + '</td>' +
				'</tr>';
		});	
		...
		$('#admin_costo_panel_instalado').val(dinero(totales.costo_por_panel, false));
		...
		$('#admin_costo_proyecto_iva').val(dinero(totales.pago_minimo_cliente, false));
		$('#admin_costo_proyecto_utilidad').val(dinero(totales.precio_al_cliente, false));
	...
	$('#generar_cotizacion').click(function(e) {
		if(revisar_cotizacion()) {
			...
			$.ajax({
				'url' : base_url + 'modificar_cotizacion/guardar_cotizacion',
				'data' : {
					'id_cotizacion_temporal' : id_cotizacion_temporal,
				...
				'type' : 'post',
				'success' : function(obj){
					window.location.replace(base_url + 'modificar_cotizacion/previsualizar_cotizacion/' + obj['id_cotizacion']);
				}
			});
		}
		else {
	...
	$('.descartar-modificaciones').click(function() {
		$.ajax({
			'url' : base_url + 'modificar_cotizacion/descartar_modificacion',
			'data' : {
				'id_cotizacion_temporal' : id_cotizacion_temporal
			},
			'beforeSend': function() {
			...
			'success' : function(obj){
				if(obj['resultado']) {
					window.location.replace(base_url + 'historial_cotizaciones');
				}
				else {
					Swal.fire({
						heightAuto: false,
						title: 'No se pudieron descartar las modificaciones', 
