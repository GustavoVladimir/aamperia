$(document).ready( function() {		
	...
	$(window).on('beforeunload', function() {
		$(window).scrollTop(0);
		/* localStorage.setItem("nombre_usuario", $('#nombre_usuario').val());
		localStorage.setItem("ubicacion", $('#ubicacion').val());
		localStorage.setItem("telefono", $('#telefono').val());
		localStorage.setItem("correo", $('#correo').val());
		localStorage.setItem("num_servicio", $('#num_servicio').val());
		localStorage.setItem("forma_calculo", $('#forma_calculo').val());
		localStorage.setItem("tarifa", $('#tarifa').val());
		localStorage.setItem("periodo", $('#periodo').val());*/
	});
	
	/*
	if (localStorage.getItem("nombre_usuario") !== null) $('#nombre_usuario').val(localStorage.getItem("nombre_usuario"));
	if (localStorage.getItem("ubicacion") !== null) $('#ubicacion').val(localStorage.getItem("ubicacion"));
	if (localStorage.getItem("telefono") !== null) $('#telefono').val(localStorage.getItem("telefono"));
	if (localStorage.getItem("correo") !== null) $('#correo').val(localStorage.getItem("correo"));
	if (localStorage.getItem("num_servicio") !== null) $('#num_servicio').val(localStorage.getItem("num_servicio"));
	*/
		...
		'success' : function(obj){
			...
			if(obj['tipos_estructura'].resultado){
				$.each(obj['tipos_estructura'].tipos, function(i, elemento) {
					$('#tipo_estructura').append('<option value=' + elemento.id_tipo_estructura + '>' + elemento.codigo + '</option>');
				});
			}
	...
	function habilitar_inputs() {
		$('#tipo_interconexion, #tipo_panel, #tipo_estructura, #sistema_monitoreo, #instalacion_electrica, #optimizador, #microinversor, #inversor_central, #agregar_estructura').prop("disabled", false); 
	}
		...
		$.ajax({
			...
			'success' : function(obj){
				...
				if(obj['resultado']){
					...
					let index = elementos_cotizacion.findIndex(item => item.id === datos_panel.id_panel && item.tipo === "panel" && item.principal === true);
					
					if(index != -1) {
						elementos_cotizacion[index].id = datos_panel.id_panel;
						elementos_cotizacion[index].codigo = datos_panel.codigo;
						elementos_cotizacion[index].producto = datos_panel.producto;
						elementos_cotizacion[index].marca = datos_panel.marca;
						elementos_cotizacion[index].cpu_mxn = round(parseFloat(datos_panel.usd_panel)*tarifa_cambio, 2);
						elementos_cotizacion[index].cpu_usd = parseFloat(datos_panel.usd_panel);
						elementos_cotizacion[index].costo_total_mxn = elementos_cotizacion[index].cpu_mxn;
					}
					else {
						let panel = {...nuevo_elemento};
						
						panel.id = datos_panel.id_panel;
						panel.codigo = datos_panel.codigo;
						panel.tipo = "panel";
						panel.producto = datos_panel.producto;
						panel.marca = datos_panel.marca;
						panel.principal = true;
						panel.cpu_usd = datos_panel.usd_panel;
						panel.cpu_mxn = round(parseFloat(datos_panel.usd_panel)*tarifa_cambio, 2);
						panel.costo_total_mxn = panel.cpu_mxn;
						
						elementos_cotizacion.push(panel);
					}
	...
	$('#num_paneles').change(function() {
		...
		let id = $('#tipo_panel').val();
		let index = elementos_cotizacion.findIndex(item => item.id == id && item.tipo == "panel" && item.principal == true);
		
		if(index != -1) {
			elementos_cotizacion[index].cantidad = cantidad;
			elementos_cotizacion[index].costo_total_mxn = round(elementos_cotizacion[index].cpu_mxn*cantidad, 2);
			...
			'success' : function(obj){
				if(obj['resultado']){
					...
					$.each(obj['estructuras'], function(i, elemento) {
						$('#estructura').append('<option value="' + elemento.id_estructura + '">' + elemento.codigo + '</option>');
					});
				}
				else {
					$('#estructura').prop("disabled", false);
	...
	$('#agregar_estructura').click(function() {
		let cantidad = parseInt($('#cantidad_estructura').val());
		let id_estructura = $('#estructura').val();
		...
		$.ajax({
			...
			'success' : function(obj){
				if(obj['resultado']) {
					...
					let nuevo = {...nuevo_elemento};
						
					nuevo.id = datos_estructura.id_estructura;
					nuevo.codigo = datos_estructura.codigo;
					nuevo.tipo = "estructura";
					nuevo.producto = datos_estructura.producto;
					nuevo.marca = datos_estructura.marca;
					nuevo.cantidad = 0;
					nuevo.principal = false;
					
					if(datos_estructura.moneda == "USD") {
						nuevo.cpu_mxn = round(parseFloat(datos_estructura.costo)*tarifa_cambio, 2);
						nuevo.cpu_usd = parseFloat(datos_estructura.costo);
					}
					else {
						nuevo.cpu_mxn = parseFloat(datos_estructura.costo);
						nuevo.cpu_usd = 0;
					}
					
					nuevo.cantidad = cantidad;
					nuevo.costo_total_mxn = round(nuevo.cpu_mxn*cantidad, 2);
					
					elementos_cotizacion.push(nuevo);
					...
					Swal.fire('Ocurrió un error', 'No se pudo cargar la estructura elegida', 'error');
				}
			}
		});
	});
	...
	*/
		...
		$.ajax({
			...
			'success' : function(obj){
				if(obj['resultado']) {
					...
					let index = elementos_cotizacion.findIndex(item => item.principal === true && item.tipo === "inversor");
					
					console.log("inversor ---");
					console.log(datos_inversor);
					console.log(elementos_cotizacion);
					console.log("index: " + index);
					
					if(index != -1) {
						elementos_cotizacion[index].id = datos_inversor.id_inversor;
						elementos_cotizacion[index].codigo = datos_inversor.codigo;
						elementos_cotizacion[index].producto = datos_inversor.producto;
						elementos_cotizacion[index].marca = datos_inversor.marca;
						elementos_cotizacion[index].principal = true;
						
						if(datos_inversor.moneda == "USD") {
							elementos_cotizacion[index].cpu_mxn = round(parseFloat(datos_inversor.costo)*tarifa_cambio, 2);
							elementos_cotizacion[index].cpu_usd = parseFloat(datos_inversor.costo);
						}
						else {
							elementos_cotizacion[index].cpu_mxn = parseFloat(datos_inversor.costo);
							elementos_cotizacion[index].cpu_usd = 0;
						}
					
						elementos_cotizacion[index].cantidad = 1;
						elementos_cotizacion[index].costo_total_mxn = elementos_cotizacion[index].cpu_mxn;
					}
					else {
						let nuevo = {...nuevo_elemento};
						
						nuevo.id = datos_inversor.id_inversor;
						nuevo.codigo = datos_inversor.codigo;
						nuevo.tipo = "inversor";
						nuevo.producto = datos_inversor.producto;
						nuevo.marca = datos_inversor.marca;
						nuevo.cantidad = 0;
						nuevo.principal = true;
						
						if(datos_inversor.moneda == "USD") {
							nuevo.cpu_mxn = round(parseFloat(datos_inversor.costo)*tarifa_cambio, 2);
							nuevo.cpu_usd = parseFloat(datos_inversor.costo);
						}
						else {
							nuevo.cpu_mxn = parseFloat(datos_inversor.costo);
							nuevo.cpu_usd = 0;
						}
						
						nuevo.cantidad = 1;
						nuevo.costo_total_mxn = nuevo.cpu_mxn;
		...
		$.ajax({
			...
			'success' : function(obj){
				if(obj['resultado']) {
					...
					let index = elementos_cotizacion.findIndex(item => item.principal === true && item.tipo === "optimizador");
					
					console.log("optimizador ---");
					console.log(datos_optimizador);
					console.log(elementos_cotizacion);
					console.log("index: " + index);
					
					if(index != -1) {
						elementos_cotizacion[index].id = datos_optimizador.id_optimizador;
						elementos_cotizacion[index].codigo = datos_optimizador.codigo;
						elementos_cotizacion[index].producto = datos_optimizador.producto;
						elementos_cotizacion[index].marca = datos_optimizador.marca;
						elementos_cotizacion[index].principal = true;
						
						if(datos_optimizador.moneda == "USD") {
							elementos_cotizacion[index].cpu_mxn = round(parseFloat(datos_optimizador.costo)*tarifa_cambio, 2);
							elementos_cotizacion[index].cpu_usd = parseFloat(datos_optimizador.costo);
						}
						else {
							elementos_cotizacion[index].cpu_mxn = parseFloat(datos_optimizador.costo);
							elementos_cotizacion[index].cpu_usd = 0;
						}
						...
						nuevo.id = datos_optimizador.id_optimizador;
						nuevo.codigo = datos_optimizador.codigo;
						nuevo.tipo = "optimizador";
						nuevo.producto = datos_optimizador.producto;
						nuevo.marca = datos_optimizador.marca;
						nuevo.cantidad = 0;
						nuevo.principal = true;
						
						if(datos_optimizador.moneda == "USD") {
							nuevo.cpu_mxn = round(parseFloat(datos_optimizador.costo)*tarifa_cambio, 2);
							nuevo.cpu_usd = parseFloat(datos_optimizador.costo);
						}
						else {
							nuevo.cpu_mxn = parseFloat(datos_optimizador.costo);
							nuevo.cpu_usd = 0;
						}
		...
		$.ajax({
			...
			'success' : function(obj){
				if(obj['resultado']) {
					...
					let index = elementos_cotizacion.findIndex(item => item.principal === true && item.tipo === "microinversor");
					
					console.log("microinversor ---");
					console.log(datos_microinversor);
					console.log(elementos_cotizacion);
					console.log("index: " + index);
					
					if(index != -1) {
						elementos_cotizacion[index].id = datos_microinversor.id_microinversor;
						elementos_cotizacion[index].codigo = datos_microinversor.codigo;
						elementos_cotizacion[index].producto = datos_microinversor.producto;
						elementos_cotizacion[index].marca = datos_microinversor.marca;
						elementos_cotizacion[index].principal = true;
						
						if(datos_microinversor.moneda == "USD") {
							elementos_cotizacion[index].cpu_mxn = round(parseFloat(datos_microinversor.costo)*tarifa_cambio, 2);
							elementos_cotizacion[index].cpu_usd = parseFloat(datos_microinversor.costo);
						}
						else {
							elementos_cotizacion[index].cpu_mxn = parseFloat(datos_microinversor.costo);
							elementos_cotizacion[index].cpu_usd = 0;
						}
						...
						nuevo.id = datos_microinversor.id_microinversor;
						nuevo.codigo = datos_microinversor.codigo;
						nuevo.tipo = "microinversor";
						nuevo.producto = datos_microinversor.producto;
						nuevo.marca = datos_microinversor.marca;
						nuevo.cantidad = 0;
						nuevo.principal = true;
						
						if(datos_microinversor.moneda == "USD") {
							nuevo.cpu_mxn = round(parseFloat(datos_microinversor.costo)*tarifa_cambio, 2);
							nuevo.cpu_usd = parseFloat(datos_microinversor.costo);
						}
						else {
							nuevo.cpu_mxn = parseFloat(datos_microinversor.costo);
							nuevo.cpu_usd = 0;
						}
		...
		$.ajax({
			...
			'success' : function(obj){
				if(obj['resultado']) {
					...
					let index = elementos_cotizacion.findIndex(item => item.principal === true && item.tipo === "sistema_monitoreo");
					
					console.log("sistema_monitoreo ---");
					console.log(datos_sistema_monitoreo);
					console.log(elementos_cotizacion);
					console.log("index: " + index);
					
					if(index != -1) {
						elementos_cotizacion[index].id = datos_sistema_monitoreo.id_sistema_monitoreo;
						elementos_cotizacion[index].codigo = datos_sistema_monitoreo.codigo;
						elementos_cotizacion[index].producto = datos_sistema_monitoreo.producto;
						elementos_cotizacion[index].marca = datos_sistema_monitoreo.marca;
						elementos_cotizacion[index].principal = true;
						
						if(datos_sistema_monitoreo.moneda == "USD") {
							elementos_cotizacion[index].cpu_mxn = round(parseFloat(datos_sistema_monitoreo.costo)*tarifa_cambio, 2);
							elementos_cotizacion[index].cpu_usd = parseFloat(datos_sistema_monitoreo.costo);
						}
						else {
							elementos_cotizacion[index].cpu_mxn = parseFloat(datos_sistema_monitoreo.costo);
							elementos_cotizacion[index].cpu_usd = 0;
						}
						...
						nuevo.id = datos_sistema_monitoreo.id_sistema_monitoreo;
						nuevo.codigo = datos_sistema_monitoreo.codigo;
						nuevo.tipo = "sistema_monitoreo";
						nuevo.producto = datos_sistema_monitoreo.producto;
						nuevo.marca = datos_sistema_monitoreo.marca;
						nuevo.cantidad = 0;
						nuevo.principal = true;
						
						if(datos_sistema_monitoreo.moneda == "USD") {
							nuevo.cpu_mxn = round(parseFloat(datos_sistema_monitoreo.costo)*tarifa_cambio, 2);
							nuevo.cpu_usd = parseFloat(datos_sistema_monitoreo.costo);
						}
						else {
							nuevo.cpu_mxn = parseFloat(datos_sistema_monitoreo.costo);
							nuevo.cpu_usd = 0;
						}
	...
	$('#mostrar_roi').on('change input', function() {
		if($(this).val() == "roi_si") {
			$('#roi_texto').removeClass('d-none');
			$('#row_roi').removeClass('d-none');
		}
		else {
			$('#roi_texto').addClass('d-none');
			$('#row_roi').addClass('d-none');
		}
	});
	...
	$('#modal_tipo_producto').on('change', function() {
					...
					'success' : function(obj){
						...
						if(obj['resultado']) {
							...
							$.each(obj['paneles'], function(i,elemento){
								var nuevaFila = tabla_agregar.row.add([
								elemento.codigo,
								elemento.producto,
								elemento.marca,
								...
								$(nuevaFila).attr('data-prod-id', elemento.id_panel);
								
								$('.cantidad-modal', nuevaFila).each(function(index,input){
									$(input).inputSpinner();
								});
							});
							*/
							...
							Swal.fire('Ocurrió un error', 'No se pudieron cargar los paneles', 'error');
						}
					}
				});
				break;
				...
				$.ajax({
					...
					'success' : function(obj){
						...
						if(obj['resultado']) {
							...
							Swal.fire('Ocurrió un error', 'No se pudieron cargar los optimizadores', 'error');
						}
					}
				});
				break;
				...
				$.ajax({
					...
					'success' : function(obj){
						...
						if(obj['resultado']) {
							...
							Swal.fire('Ocurrió un error', 'No se pudieron cargar los inversores centrales', 'error');
						}
					}
				});
				break;
				...
				$.ajax({
					...
					'success' : function(obj){
						...
						if(obj['resultado']) {
							...
							Swal.fire('Ocurrió un error', 'No se pudieron cargar las estructuras', 'error');
						}
					}
				});
				break;
				...
				$.ajax({
					...
					'success' : function(obj){
						...
						if(obj['resultado']) {
							...
							Swal.fire('Ocurrió un error', 'No se pudieron cargar los microinversores', 'error');
						}
					}
				});
				break;
				...
				$.ajax({
					...
					'success' : function(obj){
						...
						if(obj['resultado']) {
							...
							Swal.fire('Ocurrió un error', 'No se pudieron cargar los sistemas de monitoreo', 'error');
						}
					}
				});
				break;
				...
				$.ajax({
					...
					'success' : function(obj){
						...
						if(obj['resultado']) {
							...
							Swal.fire('Ocurrió un error', 'No se pudieron cargar los extras', 'error');
						}
					}
				});
				break;
		...
		function llenar_tabla() {
			...
			$.each(productos, function(i,elemento){
				let nueva_fila = tabla_agregar.row.add([
				elemento.codigo,
				elemento.producto,
				elemento.marca,
				'<input type="number" class="form-control cantidad-modal" value="1" min="1">',
	...
	$('#nuevo_extra').on('submit', function(e) {
		e.preventDefault();
		console.log("nuevo extra submit");
		if($('#extra_guardar').val() == "si") {
			$.ajax({
	...
	function actualizar_lista_elementos() {
		...
		$.each(elementos_cotizacion, function(index, producto) {
			let nueva_fila = tabla_productos.row.add([
			producto.codigo,
			producto.producto,
			producto.marca,
			...
			$(nueva_fila).attr('data-prod-id', producto.id);
			$(nueva_fila).attr('data-tipo', producto.tipo);
	...
	$('body').on('click', 'button.eliminar-producto', function() {
		let tr = $(this).closest('tr');
		let id = tr.data('prod-id');
		let tipo = tr.data('tipo');
		console.log("id: " + id + " tipo: " + tipo);
		
		let index = elementos_cotizacion.findIndex(item => item.id == id && item.tipo == tipo);
		
		console.log("index: " + index);
		
		if(index != -1) {
			let producto = elementos_cotizacion[index];
			elementos_cotizacion.splice(index, 1);
			
			switch(producto.tipo) {
				case 'inversor' :
					$('#inversor_central option:eq(0)').prop('selected', true);
					break;
				case 'microinversor' :
					$('#microinversor option:eq(0)').prop('selected', true);
					break;
				case 'optimizador' :
					$('#optimizador option:eq(0)').prop('selected', true);
					break;
				case 'sistema_monitoreo' :
					$('#sistema_monitoreo option:eq(0)').prop('selected', true);
					break;
				case 'panel' :
					$('#tipo_panel option:eq(0)').prop('selected', true);
					break;
			}
