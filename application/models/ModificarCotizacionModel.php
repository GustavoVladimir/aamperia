<?php
	class ModificarCotizacionModel extends CI_Model {
		...
		public function precargar_cotizacion($id_cotizacion, $datos_usuario) {
			$query = $this->db->query("SELECT * FROM cotizaciones WHERE id_cotizacion = ?", $id_cotizacion);
			...
			if($query->num_rows() > 0) {
				...
				$query = $this->db->query("SELECT id_cotizacion_temporal FROM cotizaciones_temporales WHERE id_cotizacion_existente = ?", $id_cotizacion);
			
				if($query->num_rows() > 0) {
					$id_cotizacion_temporal_viejo = $query->row_array()['id_cotizacion_temporal'];
					
					$this->db->query("DELETE FROM cotizaciones_temporales_ahorro WHERE id_cotizacion_temporal = ?", $id_cotizacion_temporal_viejo);
					
					$this->db->query("DELETE FROM cotizaciones_productos_temporales WHERE id_cotizacion_temporal = ?", $id_cotizacion_temporal_viejo);
					
					$this->db->query("DELETE FROM cotizaciones_temporales WHERE id_cotizacion_temporal = ?", $id_cotizacion_temporal_viejo);
				}
				
				$this->db->query("INSERT INTO cotizaciones_temporales (id_usuario, nombre, ubicacion, telefono, correo, numero_servicio, fecha_cotizacion, tarifa, periodo, forma_calculo, recibo_1, recibo_2, recibo_3, recibo_4, recibo_5, recibo_6, consumo_promedio_kwh, consumo_promedio_precio, gasto_anual_cfe, gasto_bimestral_cfe, costo_suministro, dap, eficiencia, hps, periodo_dias, indice_utilidad, costo_metro, tasa_cambio, tasa_iva, fecha_tasa, tipo_interconexion, paneles_elegidos, num_paneles, material_electrico, instalacion, tuberia_cableado, potencia_total, porcentaje_produccion, produccion_periodo, produccion_anual, costo_por_panel, costo_por_watt, costo_proveedores, iva_proveedores, utilidad, pago_minimo_cliente, precio_al_cliente, precio_final, porcentaje_minimo, subtotal, iva, total, retorno_inversion, mostrar_roi, metros_instalacion, grafica_consumo, grafica_roi, id_cotizacion_existente) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", array($datos_cotizacion['id_usuario'], $datos_cotizacion['nombre'], $datos_cotizacion['ubicacion'], $datos_cotizacion['telefono'], $datos_cotizacion['correo'], $datos_cotizacion['numero_servicio'], $datos_cotizacion['fecha_cotizacion'], $datos_cotizacion['tarifa'], $datos_cotizacion['periodo'], $datos_cotizacion['forma_calculo'], $datos_cotizacion['recibo_1'], $datos_cotizacion['recibo_2'], $datos_cotizacion['recibo_3'], $datos_cotizacion['recibo_4'], $datos_cotizacion['recibo_5'], $datos_cotizacion['recibo_6'], $datos_cotizacion['consumo_promedio_kwh'], $datos_cotizacion['consumo_promedio_precio'], $datos_cotizacion['gasto_anual_cfe'], $datos_cotizacion['gasto_bimestral_cfe'], $datos_cotizacion['costo_suministro'], $datos_cotizacion['dap'], $datos_cotizacion['eficiencia'], $datos_cotizacion['hps'], $datos_cotizacion['periodo_dias'], $datos_cotizacion['indice_utilidad'], $datos_cotizacion['costo_metro'], $datos_cotizacion['tasa_cambio'], $datos_cotizacion['tasa_iva'], $datos_cotizacion['fecha_tasa'], $datos_cotizacion['tipo_interconexion'], $datos_cotizacion['paneles_elegidos'], $datos_cotizacion['num_paneles'], $datos_cotizacion['material_electrico'], $datos_cotizacion['instalacion'], $datos_cotizacion['tuberia_cableado'], $datos_cotizacion['potencia_total'], $datos_cotizacion['porcentaje_produccion'], $datos_cotizacion['produccion_periodo'], $datos_cotizacion['produccion_anual'], $datos_cotizacion['costo_por_panel'], $datos_cotizacion['costo_por_watt'], $datos_cotizacion['costo_proveedores'], $datos_cotizacion['iva_proveedores'], $datos_cotizacion['utilidad'], $datos_cotizacion['pago_minimo_cliente'], $datos_cotizacion['precio_al_cliente'], $datos_cotizacion['precio_final'], $datos_cotizacion['porcentaje_minimo'], $datos_cotizacion['subtotal'], $datos_cotizacion['iva'], $datos_cotizacion['total'], $datos_cotizacion['retorno_inversion'], $datos_cotizacion['mostrar_roi'], $datos_cotizacion['metros_instalacion'], $datos_cotizacion['grafica_consumo'], $datos_cotizacion['grafica_roi'], $id_cotizacion));
				
				$id_cotizacion_temporal = $this->db->insert_id();
				
				foreach($productos as $producto) {
					$this->db->query("INSERT INTO cotizaciones_productos_temporales (id_cotizacion_temporal, cantidad, cpu_usd, cpu_mxn, costo_total, precio_unitario, precio_final, principal, id_producto, tipo_producto) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", array($id_cotizacion_temporal, $producto['cantidad'], $producto['cpu_usd'], $producto['cpu_mxn'], $producto['costo_total'], $producto['precio_unitario'], $producto['precio_final'], $producto['principal'], $producto['id_producto'], $producto['tipo_producto']));					
				}
				
				foreach($datos_ahorro as $dato_ahorro) {
					$this->db->query("INSERT INTO cotizaciones_temporales_ahorro (id_cotizacion_temporal, periodo, actual, con_aamperia, ahorro) VALUES (?, ?, ?, ?, ?)", array($id_cotizacion_temporal, $dato_ahorro['periodo'], $dato_ahorro['actual'], $dato_ahorro['con_aamperia'], $dato_ahorro['ahorro']));			
				}
				...
				if ($this->db->trans_status() === FALSE) {
					...
					$obj['resultado'] = true;
					$obj['id_cotizacion_temporal'] = $id_cotizacion_temporal;
					$datos_usuario['id_cotizacion_temporal_modificar'] = $id_cotizacion_temporal; 
		...
		public function cargar_cotizacion_temporal($datos_usuario) {
			$query = $this->db->query("SELECT id_usuario FROM usuarios WHERE id_usuario = ?", $datos_usuario['id_usuario']);
			
			if($query->num_rows() > 0) {
				$id_usuario = $datos_usuario['id_usuario'];
				$id_cotizacion_temporal_modificar = $datos_usuario['id_cotizacion_temporal_modificar'];
				
				$query = $this->db->query("SELECT * FROM cotizaciones_temporales WHERE id_cotizacion_temporal = ?", $id_cotizacion_temporal_modificar);
			
				if($query->num_rows() > 0) {
					$obj['tipo_cotizacion'] = "existente";
					$obj['cotizacion_temporal'] = $query->row_array();
					$calcular_totales = $this->calcular_totales($id_cotizacion_temporal_modificar);
					if($calcular_totales['resultado']) {
						...
						if($obj['cotizacion_temporal']['paneles_elegidos'] == 1) {
							$obj['datos_ahorro'] = $this->calcular_ahorros($id_cotizacion_temporal_modificar, (float)$obj['cotizacion_temporal']['produccion_periodo'], $obj['cotizacion_temporal'], $obj['totales']['total_final']);
						}
					}
					else {
						$obj['resultado'] = false;
		...
		public function guardar_instalacion_electrica($id_cotizacion_temporal, $metros, $instalacion) {
			...
			if($query->num_rows() > 0) {
				...
				$query = $this->db->query("SELECT * FROM productos_fijos WHERE codigo = 'TUBERIA_INSTALACION'");
		...
		public function guardar_cotizacion($id_cotizacion_temporal, $grafica_consumo, $grafica_roi, $datos_usuario) {
			$query = $this->db->query("SELECT * FROM cotizaciones_temporales WHERE id_cotizacion_temporal = ?", $id_cotizacion_temporal);
			
			if($query->num_rows() > 0) {
				$datos_cotizacion = $query->row_array();
				$id_cotizacion_existente = $datos_cotizacion['id_cotizacion_existente'];
				...
				$this->db->query("DELETE FROM cotizaciones_ahorro WHERE id_cotizacion = ?", $id_cotizacion_existente);
				
				$this->db->query("DELETE FROM cotizaciones_productos WHERE id_cotizacion = ?", $id_cotizacion_existente);
				
				$this->db->query("UPDATE cotizaciones SET estado_cotizacion = ?, nombre = ?, ubicacion = ?, telefono = ?, correo = ?, numero_servicio = ?, fecha_cotizacion = ?, tarifa = ?, periodo = ?, forma_calculo = ?, recibo_1 = ?, recibo_2 = ?, recibo_3 = ?, recibo_4 = ?, recibo_5 = ?, recibo_6 = ?, consumo_promedio_kwh = ?, consumo_promedio_precio = ?, gasto_anual_cfe = ?, gasto_bimestral_cfe = ?, costo_suministro = ?, dap = ?, eficiencia = ?, hps = ?, periodo_dias = ?, indice_utilidad = ?, costo_metro = ?, tasa_cambio = ?, tasa_iva = ?, fecha_tasa = ?, tipo_interconexion = ?, paneles_elegidos = ?, num_paneles = ?, material_electrico = ?, instalacion = ?, tuberia_cableado = ?, potencia_total = ?, porcentaje_produccion = ?, produccion_periodo = ?, produccion_anual = ?, costo_por_panel = ?, costo_por_watt = ?, costo_proveedores = ?, iva_proveedores = ?, utilidad = ?, pago_minimo_cliente = ?, precio_al_cliente = ?, precio_final = ?, porcentaje_minimo = ?, subtotal = ?, iva = ?, total = ?, retorno_inversion = ?, mostrar_roi = ?, metros_instalacion = ?, grafica_consumo = ?, grafica_roi = ? WHERE id_cotizacion = ?", array('Incompleto', $datos_cotizacion['nombre'], $datos_cotizacion['ubicacion'], $datos_cotizacion['telefono'], $datos_cotizacion['correo'], $datos_cotizacion['numero_servicio'], $datos_cotizacion['fecha_cotizacion'], $datos_cotizacion['tarifa'], $datos_cotizacion['periodo'], $datos_cotizacion['forma_calculo'], $datos_cotizacion['recibo_1'], $datos_cotizacion['recibo_2'], $datos_cotizacion['recibo_3'], $datos_cotizacion['recibo_4'], $datos_cotizacion['recibo_5'], $datos_cotizacion['recibo_6'], $datos_cotizacion['consumo_promedio_kwh'], $datos_cotizacion['consumo_promedio_precio'], $datos_cotizacion['gasto_anual_cfe'], $datos_cotizacion['gasto_bimestral_cfe'], $datos_cotizacion['costo_suministro'], $datos_cotizacion['dap'], $datos_cotizacion['eficiencia'], $datos_cotizacion['hps'], $datos_cotizacion['periodo_dias'], $datos_cotizacion['indice_utilidad'], $datos_cotizacion['costo_metro'], $datos_cotizacion['tasa_cambio'], $datos_cotizacion['tasa_iva'], $datos_cotizacion['fecha_tasa'], $datos_cotizacion['tipo_interconexion'], $datos_cotizacion['paneles_elegidos'], $datos_cotizacion['num_paneles'], $datos_cotizacion['material_electrico'], $datos_cotizacion['instalacion'], $datos_cotizacion['tuberia_cableado'], $datos_cotizacion['potencia_total'], $datos_cotizacion['porcentaje_produccion'], $datos_cotizacion['produccion_periodo'], $datos_cotizacion['produccion_anual'], $datos_cotizacion['costo_por_panel'], $datos_cotizacion['costo_por_watt'], $datos_cotizacion['costo_proveedores'], $datos_cotizacion['iva_proveedores'], $datos_cotizacion['utilidad'], $datos_cotizacion['pago_minimo_cliente'], $datos_cotizacion['precio_al_cliente'], $datos_cotizacion['precio_final'], $datos_cotizacion['porcentaje_minimo'], $datos_cotizacion['subtotal'], $datos_cotizacion['iva'], $datos_cotizacion['total'], $datos_cotizacion['retorno_inversion'], $datos_cotizacion['mostrar_roi'], $datos_cotizacion['metros_instalacion'], $grafica_consumo, $grafica_roi, $id_cotizacion_existente));
				
				foreach($productos as $producto) {
					$this->db->query("INSERT INTO cotizaciones_productos (id_cotizacion, cantidad, cpu_usd, cpu_mxn, costo_total, precio_unitario, precio_final, principal, id_producto, tipo_producto) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)", array($id_cotizacion_existente, $producto['cantidad'], $producto['cpu_usd'], $producto['cpu_mxn'], $producto['costo_total'], $producto['precio_unitario'], $producto['precio_final'], $producto['principal'], $producto['id_producto'], $producto['tipo_producto']));					
				}
				
				foreach($datos_ahorro as $dato_ahorro) {
					$this->db->query("INSERT INTO cotizaciones_ahorro (id_cotizacion, periodo, actual, con_aamperia, ahorro) VALUES (?, ?, ?, ?, ?)", array($id_cotizacion_existente, $dato_ahorro['periodo'], $dato_ahorro['actual'], $dato_ahorro['con_aamperia'], $dato_ahorro['ahorro']));					
				}
				...
				if ($this->db->trans_status() === FALSE) {
					...
					$this->db->trans_commit();
					$obj['resultado'] = true;
					$obj['id_cotizacion'] = $id_cotizacion_existente;
					$datos_usuario['id_cotizacion_temporal_modificar'] = null; 
					$obj['usuario'] = $datos_usuario;
				}
			}
			else {
		...
		public function descartar_modificacion($id_cotizacion_temporal, $datos_usuario) {		
			$query = $this->db->query("SELECT id_cotizacion_temporal FROM cotizaciones_temporales WHERE id_cotizacion_temporal = ?", $id_cotizacion_temporal);
